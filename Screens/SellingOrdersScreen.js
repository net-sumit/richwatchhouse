...
const SellingOrdersScreen = ({ route, navigation }) => {

    // For language translate
    const { t, i18n } = useTranslation();

    const [isLoading, setLoading] = useState(true);
    const sellingOrdersApi = API_URL + 'user-selling-orders.php';
    const [sellingOrdersData, setSellingOrdersData] = useState([]);
    ...
    useEffect(() => {
        ...
        const asyncPostCall = async () => {
            try {
                const response = await fetch(sellingOrdersApi, {
                ...
                const data = await response.json();
                    ...
                    setLoading(false);
                    setSellingOrdersData(data.data);
        ...
        <>
            ...
            <View style={{ flex: 1, backgroundColor: "#FFF", padding: 10, }}>
                ...
                <View style={styles.SellingOrderArea}>
                    <Text style={styles.allAuctionTitle}>{t("selling_orders")}</Text>
                    {sellingOrdersData ?
                        <View style={styles.SellingOrderContentArea}>
                            {isLoading ? <ActivityIndicator size="large" style={{ marginTop: 300 }} /> : (
                                ...
                                <ScrollView>
                                    ...
                                    {
                                        sellingOrdersData.map((item, index) => {
                                            return (
                                                <View style={styles.SellingOrderContentAreaSlide} key={index}>
                                                    ...
                                                    <View style={styles.TopTitleAreaTitleCont}>
                                                        <Text style={styles.TopTitle}>{t("Product")}: <Text style={styles.TopTitleAreaTitleCont}>{item.order_item}</Text></Text>
                                                    </View>
                                                    <View style={styles.TopTitleAreaTitleCont}>
                                                        <Text style={styles.TopTitle}>{t("Buyer")}: <Text style={styles.TopTitleAreaTitleCont}>{item.buyer_name}</Text></Text>
                                                    </View>
                                                    <View style={styles.TopTitleAreaTitleCont}>
                                                        <Text style={styles.TopTitle}>{t("Total")}: 
                                                            <RenderHtml
                                                    ...
                                                    <View style={styles.TopTitleAreaTitleCont}>
                                                        <Text style={styles.TopTitle}>{t("Order_Status")}: <Text style={styles.TopTitleAreaTitleCont}>{item.order_status}</Text></Text>
                                                        <Text style={styles.TopTitle}>{t("Your_Payment")}: <Text style={styles.TopTitleAreaTitleCont}>{item.seller_payment_status}</Text></Text>
                                                    </View>
...
export default SellingOrdersScreen